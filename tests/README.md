...
Core tests take longer than any other APEPEPOW tests, due to the high amount of computational work involved in validating core components.
...
To run only APEPEPOW's core tests (after building):
...
To run only APEPEPOW's crypto tests (after building):
...
```bash
apepepowd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `apepepow-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only APEPEPOW's hash tests (after building):
...
To run only APEPEPOW's performance tests (after building):
...
To run only APEPEPOW's unit tests (after building):
