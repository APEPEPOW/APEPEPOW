...
# a main offline apepepowd, does most of the tests
# a restricted RPC apepepowd setup with RPC payment
# two local online apepepowds connected to each other
N_APEPEPOWDS = 4

# 4 wallets connected to the main offline apepepowd
# 1 wallet connected to the first local online apepepowd
# 1 offline wallet
N_WALLETS = 6
...
apepepowd_base = [builddir + "/bin/apepepowd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "apepepowd_p2p_port", "--rpc-bind-port", "apepepowd_rpc_port", "--zmq-rpc-bind-port", "apepepowd_zmq_port", "--zmq-pub", "apepepowd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "apepepowd_data_dir", "--log-level", "1"]
apepepowd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/apepepow-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_APEPEPOWDS):
  command_lines.append([str(18180+i) if x == "apepepowd_rpc_port" else str(18280+i) if x == "apepepowd_p2p_port" else str(18380+i) if x == "apepepowd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "apepepowd_zmq_pub" else builddir + "/functional-tests-directory/apepepowd" + str(i) if x == "apepepowd_data_dir" else x for x in apepepowd_base])
  if i < len(apepepowd_extra):
    command_lines[-1] += apepepowd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/apepepowd' + str(i) + '.log', 'a+'))
