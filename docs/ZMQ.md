# The Current/Future Status of ZMQ in APEPEPOW

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. APEPEPOW
...
matching supported by both APEPEPOW and ZMQ. The `format`, `context` and `event`
...
The APEPEPOW daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The APEPEPOW daemon also ensures that
...
The APEPEPOW daemon will send a `txpool_add` pub exactly once for each
