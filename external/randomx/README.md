...
The first audit was generously funded by [Arweave](https://www.arweave.org/), one of the early adopters of RandomX. The remaining three audits were funded by donations from the [APEPEPOW community](https://ccs.getapepepow.org/proposals/RandomX-audit.html). All four audits were coordinated by [OSTIF](https://ostif.org/).

Final reports from all four audits are available in the [audits](audits/) directory. None of the audits found any critical vulnerabilities, but several changes in the algorithm and the code were made as a direct result of the audits. More details can be found in the [final report by OSTIF](https://ostif.org/four-audits-of-randomx-for-apepepow-and-arweave-have-been-completed-results/).
...
RandomX was primarily designed as a PoW algorithm for [APEPEPOW](https://www.getapepepow.org/). The recommended usage is following:
...
RandomX was successfully activated on the APEPEPOW network on the 30th November 2019.
...
**Note**: To achieve ASIC resistance, the key `K` must change and must not be miner-selectable. We recommend to use blockchain data as the key in a similar way to the APEPEPOW example above. If blockchain data cannot be used for some reason, use a predefined sequence of keys.

### CPU performance
The table below lists the performance of selected CPUs using the optimal number of threads (T) and large pages (if possible), in hashes per second (H/s). "CNv4" refers to the CryptoNight variant 4 (CN/R) hashrate measured using [APEPEPOWig](https://github.com/APEPEPOWig/APEPEPOWig) v2.14.1. "Fast mode" and "Light mode" are the two modes of RandomX.
...
The code from the above repositories is included in the open source miner [APEPEPOWig](https://github.com/APEPEPOWig/APEPEPOWig).
...
Author's APEPEPOW address:
```
845xHUh5GvfHwc2R8DVJCE7BT2sd4YEcmjG8GNSdmeNsP5DTEjXd1CNgxTcjHjiFuthRHAoVEJjM7GyKzQKLJtbd56xbh7V
```
Total donations received: ~3.86 APEPEPOW (as of 30th August 2019). Thanks to all contributors.
