...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/apepepowd"
  then
    apepepowd="$dir/apepepowd"
    break
  fi
done
if test -z "$apepepowd"
then
  echo "apepepowd not found"
  exit 1
fi
echo "Found: $apepepowd"

TORDIR="$DIR/apepepow-over-tor"
TORRC="$TORDIR/torrc"
...
"$apepepowd" \
  ...
  --add-priority-node 2APEPEPOWnode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$apepepowd" status)
...
if test "$ready" = 0
then
  echo "Error starting apepepowd"
  tail -n 400 "$HOME/.bitapepepow/bitapepepow.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*APEPEPOW\ \'" | tail -n 20
  exit 1
fi
