...
gsigs = 'https://github.com/apepepow-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('apepepow'):
        # Remove the potentially stale apepepow dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'apepepow', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'apepepow'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/apepepow/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'apepepow='+args.commit, '--url', 'apepepow='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/apepepow-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/apepepow/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/apepepow/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/apepepow-project/apepepow', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/apepepow', exist_ok=True)
    os.chdir('builder/inputs/apepepow')
    if args.pull:
